...
export const usePurchasePolicy = ({
  ...
  useEffect(() => {
    ...
    if (isGreaterOrEqual(value || 0, availableLiquidity || 0)) {
      ...
      setError(
        t`Maximum protection available is ${maxProtection}. Choose a amount less than available.`
      );
    ...
    if (
      isGreater(
        convertToUnits(MIN_PROPOSAL_AMOUNT, liquidityTokenDecimals),
        convertToUnits(value, liquidityTokenDecimals) || 0
      )
    ) {
      ...
      setError(
        t`Minimum proposal amount should be greater than ${minProposal}`
      );
    ...
    if (
      isGreater(
        convertToUnits(value, liquidityTokenDecimals) || 0,
        convertToUnits(MAX_PROPOSAL_AMOUNT, liquidityTokenDecimals)
      )
    ) {
      const maxProposal = formatCurrency(
        MAX_PROPOSAL_AMOUNT,
        router.locale,
        liquidityTokenSymbol,
        true
      ...
      setError(t`Maximum proposal amount should be less than ${maxProposal}`);
    ...
    try {
      ...
      const onTransactionResult = async (tx) => {
        ...
        const logData = {
          ...
          coveragePeriodMonthFormatted: getMonthNames(router.locale)[
            (currentMonthIndex - 1 + parseInt(coverMonth)) % 12
          ],
          coveragePeriodYear:
            (currentMonthIndex + parseInt(coverMonth)) % 12 === 0
              ? year
              : year + 1,
