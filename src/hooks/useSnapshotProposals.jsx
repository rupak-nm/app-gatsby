...
const parseProposalsData = (data, locale) => {
  ...
  const proposals = data.proposals.map((proposal) => {
    const scoresSum = proposal.scores.reduce((acc, curr) => {
      return acc + curr;
    }, 0);
    const scores = proposal.choices.map((choice, i) => {
      ...
      return {
        name: choice,
        value: score
          ? formatCurrency(score, locale, proposal.symbol, true).short
          : `0 ${proposal.symbol}`,
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (jsonData.data) {
            ...
            setData((prev) => {
              return page > 1 ? [...prev, ...latestData] : latestData;
            });
            setLastFetchedLength(latestData.length);
            if (jsonData.data.space) {
              setTotal(jsonData.data.space.proposalsCount);
            }
          }
        }
      } catch (error) {
