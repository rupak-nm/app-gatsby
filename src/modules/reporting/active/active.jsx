...
const sorterData = {
  ...
  [SORT_TYPES.UTILIZATION_RATIO]: {
    selector: (report) => {
      return report.stats.utilization;
    },
...
export const ReportingActivePage = () => {
  ...
  const { searchValue, setSearchValue, filtered } = useSearchResults({
    ...
    filter: (report, term) => {
      return toStringSafe(report.stats.text).includes(toStringSafe(term));
    },
  });
        ...
        <Grid className="mb-24 mt-14" data-testid="active-page-grid">
          {data.map((report) => {
            ...
            const coverOrProductData = isDiversified
              ? getProduct(report.coverKey, report.productKey)
              : getCoverByCoverKey(report.coverKey);

            return (
              <Link
                to={Routes.ViewReport(
                  report.coverKey,
                  report.productKey,
                  report.incidentDate
                )}
