...
const sorterData = {
  [SORT_TYPES.ALPHABETIC]: {
    selector: (report) => {
      return report.stats.text;
    },
    datatype: SORT_DATA_TYPES.STRING,
  },
  [SORT_TYPES.INCIDENT_DATE]: {
    selector: (report) => {
      return report.stats.incidentDate;
    },
    datatype: SORT_DATA_TYPES.BIGNUMBER,
  },
  [SORT_TYPES.RESOLVED_DATE]: {
    selector: (report) => {
      return report.stats.resolvedOn;
    },
    datatype: SORT_DATA_TYPES.BIGNUMBER,
  },
};
...
export const ReportingResolvedPage = () => {
  ...
  console.log(router.push, typeof router.push);
  const {
  ...
  const { searchValue, setSearchValue, filtered } = useSearchResults({
    ...
    filter: (item, term) => {
      return toStringSafe(item.stats.text).indexOf(toStringSafe(term)) > -1;
    },
  });
    ...
    <Container className="pt-16 pb-36">
      ...
      {loading || dataLoading ? (
        ...
        <div className="mt-6">
          <TableWrapper>
            <Table>
              <THead rowClass="border-t-0" columns={columns} />
              <tbody
              ...
              >
                ...
                {resolvedReportsWithData.map(
                  ({ report, coverOrProductData }) => {
