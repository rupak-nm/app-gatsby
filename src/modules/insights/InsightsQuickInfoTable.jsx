...
const RenderUtilizationRatio = ({ ratio, locale }) => {
  return (
    <td className="px-6 py-2.5 text-sm leading-5 text-01052D">
      <div
  ...
  return [
    ...
    },
    {
      ...
      renderData: (row, { locale }) => {
        return (
          <RenderUtilizationRatio
            ratio={row.utilizationRatio}
            locale={locale}
          />
        );
      },
    },
    {
      ...
      renderData: (row, { locale, liquidityTokenDecimals }) => {
        return (
          <RenderCapacity
...
export const InsightsQuickInfoTable = () => {
  ...
  const topCovers = useMemo(() => {
    const products = getAllProducts().filter((x) => {
      return toBN(x.utilizationRatio).isGreaterThanOrEqualTo(0.7);
    });
  ...
  const paginatedData = topCovers.slice(
