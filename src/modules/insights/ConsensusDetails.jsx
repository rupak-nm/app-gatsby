...
function ConsensusDetails({ consensusIndex, setConsensusIndex, data }) {
  ...
  const href = Routes.ViewReport(
    coverKey,
    productKey,
    coverOrProductData?.activeIncidentDate
  );
  ...
  const efficiency = formatPercent(
    toBN(coverOrProductData.capitalEfficiency).dividedBy(MULTIPLIER).toString()
  );
  ...
  if (isDiversified) {
    spillover = toBN(protection).isGreaterThanOrEqualTo(totalPoolAmount)
      ? toBN(protection).minus(totalPoolAmount)
      : toBN("0");
    ...
    <div>
          ...
          <div className="flex items-center justify-between lg:justify-start">
            <div className="flex items-center">
              <img
                ...
                // @ts-ignore
                onError={(ev) => {
                  return (ev.target.src = "/images/covers/empty.svg");
                }}
              />
              <div className="mr-6 text-sm">{projectOrProductName}</div>
            </div>
            <PreviousNext
              hasNext={consensusIndex < data.incidentReports.length - 1}
              hasPrevious={consensusIndex > 0}
              onPrevious={() => {
      ...
      <div className="flex flex-wrap items-start my-6 gap-x-6 gap-y-10 lg:my-10">
        <StatsCard
          ...
          title="Liquidity"
          value={liquidityText.short}
          tooltip={liquidityText.long}
        />
        <StatsCard
          titleClass="text-999BAB"
          title="Utilization"
          value={formatPercent(utilization, router.locale)}
        />
        <StatsCard
          titleClass="text-999BAB"
          title="Exposure"
          value={formattedProtection.short}
          tooltip={formattedProtection.long}
        />
        <StatsCard
          titleClass="text-999BAB"
          title="Spillover"
          value={isDiversified ? spillOverText.short : "DEDI"}
        ...
        <StatsCard
          titleClass="text-999BAB"
          title="NPM Staked"
          value={totalStakeText.short.split(" ")[0]}
          tooltip={totalStakeText.long}
        />
        <StatsCard
          titleClass="text-999BAB"
          title="Guaranteed"
          value={isDiversified ? "No" : "Yes"}
        />
        <StatsCard
          titleClass="text-999BAB"
          title="Mode"
          value={isDiversified ? "FC-FS" : "-"}
        />
      </div>
      ...
      <div className="flex items-center my-6 text-xs lg:my-10">
        ...
        <Badge className="px-1 border-none text-364253 bg-F3F5F7">
      ...
      <div className="grid items-center text-sm grid-cols-auto-1fr gap-x-4 gap-y-6">
        ...
        <div>
          <div
            className="h-2 rounded-full bg-21AD8C"
            style={{
        ...
        <div>
          <div
            className="h-2 rounded-full bg-FA5C2F"
            style={{
