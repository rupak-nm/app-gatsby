  ...
  return (
    <div>
      {loading ? (
        ...
        <StatDisplay router={router} statsData={statsData} />
      )}
    </div>
  );
};
  ...
  return (
    <div className="flex flex-wrap items-start justify-between pb-6 lg:pb-10 gap-x-2 gap-y-4">
      <StatsCard
        ...
        value={
          formatCurrency(statsData?.combined?.totalCapacity || 0, router.locale)
            .short
        }
        tooltip={
          formatCurrency(statsData?.combined?.totalCapacity || 0, router.locale)
      ...
      <StatsCard
        ...
        title="Covered"
        value={
          formatCurrency(statsData?.combined?.totalCoveredAmount, router.locale)
            .short
        }
        tooltip={
          formatCurrency(statsData?.combined?.totalCoveredAmount, router.locale)
      ...
      <StatsCard
        ...
        title="Cover Fee"
        value={
          formatCurrency(statsData?.combined?.totalCoverFee, router.locale)
            .short
        }
        tooltip={
          formatCurrency(statsData?.combined?.totalCoverFee, router.locale).long
        }
      />
    </div>
  );
