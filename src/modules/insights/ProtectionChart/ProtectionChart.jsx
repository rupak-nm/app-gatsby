...
export const ProtectionChart = ({
    ...
    return {
      ...
      datasets: Object.keys(data).map((chain) => {
        return {
          ...
          data: data[chain].map((item) => {
            return parseFloat(item[dataKey]);
          }),
          backgroundColor: ["1", "84531"].includes(chain)
            ? "#4E7DD9"
            : "#21AD8C",
          barPercentage: 1,
          borderWidth: 0,
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      tooltip: {
        enabled: false,
        external: (context, className) => {
          return externalTooltipHandler(context, className, dataKey);
        },
        displayColors: false,
        callbacks: {
          title: function () {
          ...
          {chains.map((chain) => {
            return (
              <div className="flex items-center gap-1" key={chain.value}>
                <div
                  className={
                    "rounded-full h-3.5 w-3.5 bg-" +
                    (ChainAnalyticsColors[chain.value] ||
                      ChainAnalyticsColors.DEFAULT)
                  }
                />
