...
const RenderNetwork = ({ networkId }) => {
  ...
  return (
    <td className="px-6 py-4">
      <div
        className="flex flex-row text-sm leading-5 w-54 text-01052D"
        title={networkName}
      >
        <LogoIcon
          width="24"
...
const RenderCover = ({ coverFee }) => {
  ...
  return (
    <td className="px-6 py-4 text-sm leading-5 text-01052D">
      <span title={formatCurrency(coverFee, router.locale).long}>
...
const RenderCapacity = ({ capacity }) => {
  ...
  return (
    <td className="px-6 py-4 text-sm leading-5 text-right text-01052D">
      <span title={formatCurrency(capacity, router.locale).long}>
...
const RenderTVL = ({ tvl }) => {
  ...
  return (
    <td className="px-6 py-4 text-sm leading-5 text-right text-01052D">
      <span title={formatCurrency(tvl, router.locale).long}>
  ...
  return [
    {
      ...
      renderHeader: (col) => {
        return renderHeader(
          ...
          null,
          null,
          "xs:text-999BAB lg:text-404040"
        );
      },
      renderData: (row) => {
        return <RenderNetwork networkId={row.networkId} />;
      },
    },
    {
      id: "Cover Fee Earned",
      ...
      renderData: (row) => {
        return <RenderCover coverFee={row.coverFee} />;
      },
    },
    {
      id: "TVL",
    ...
    },
    {
      ...
      renderData: (row) => {
        return <RenderCapacity capacity={row.capacity} />;
      },
    },
  ];
};
    ...
    <div>
      ...
      <TableWrapper className="mt-0">
        <Table>
          ...
          <TBody isLoading={loading} columns={columns} data={data} />
        </Table>
      </TableWrapper>
    </div>
  );
