...
export const PolicyCalculation = ({ feeData, loading, selected, amount }) => {
  const router = useRouter();
  const { liquidityTokenDecimals, liquidityTokenSymbol } = useAppConstants();

  const coverFee = convertFromUnits(
    feeData?.fee || "",
  ...
  const handleBuyCover = () => {
    ...
    if (amount) {
      routerObject.query = { amount };
    }

    const url = getUrlFromQueryObject(
      routerObject.query,
      routerObject.pathname
    );
    router.push(url);
  };
