...
export const DateRangePicker = ({
  ...
  return (
    <div className="relative flex">
      <div
        className="absolute h-2 bg-999BAB  bg-opacity-30 top-1.5 z-5"
        style={{ width: "calc(100% - 20px)" }}
      />
      <div
        className="absolute h-2 bg-4E7DD9 border-l-white border-l-2 z-5 border-r-white border-r-2 top-1.5"
...
const DateRangerPickerRadio = ({
  ...
  return (
    ...
    >
      ...
      <div
        className={classNames(
          "w-6 h-6 bg-EEEEEE rounded-full z-5 absolute flex align-middle justify-center"
        )}
      >
        {" "}
        {rest.checked && (
          <div
            className={classNames("w-3 h-3 bg-4E7DD9 rounded-full my-auto")}
          />
        )}{" "}
      </div>
      ...
      <label
        className={classNames(
          "cursor-pointer text-xs uppercase flex-1 text-01052D",
          labelClass,
          disabled && "cursor-not-allowed"
        )}
        htmlFor={id}
