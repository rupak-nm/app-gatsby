...
export const CalculatorCard = () => {
  ...
  function handleChange(val) {
    ...
    if (isGreater(val, MAX_PROPOSAL_AMOUNT)) {
      const maxProposalThreshold = formatCurrency(
        MAX_PROPOSAL_AMOUNT,
        router.locale,
        liquidityTokenSymbol,
        true
      ).long;
      error = t`Maximum proposal threshold is ${maxProposalThreshold}`;
    } else if (
      isGreater(availableLiquidity, 0) &&
      isGreaterOrEqual(val, availableLiquidity)
    ) {
      const maxProtection = formatCurrency(
        availableLiquidity,
        router.locale,
        liquidityTokenSymbol,
        true
      ).long;
      error = t`Maximum protection available is ${maxProtection}`;
    } else if (isGreater(MIN_PROPOSAL_AMOUNT, val)) {
      const minProposal = formatCurrency(
        MIN_PROPOSAL_AMOUNT,
        router.locale,
        liquidityTokenSymbol,
        true
      ).long;
      error = t`Minimum proposal threshold is ${minProposal}`;
    }
  ...
  const calculatePolicyFee = async () => {
    ...
    const { data, error } = await calculateCoverPolicyFee({
      ...
      productKey:
        isValidProduct(selectedCover?.productKey) &&
        (selectedCover?.productKey || ""),
      coverMonth,
      liquidityTokenDecimals,
    });
    if (error) {
  ...
  const buttonClass =
    "block w-full pt-3 pb-3 uppercase px-4 py-0 text-sm font-semibold tracking-wider leading-loose text-white border border-transparent rounded-md whitespace-nowrap hover:bg-opacity-90 disabled:cursor-not-allowed disabled:opacity-75 disabled:hover:bg-opacity-100 bg-primary";
