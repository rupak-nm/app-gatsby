...
export const CalculatorAmountHandler = ({
  ...
  useEffect(() => {
    if (
      !inputProps.value ||
      inputProps.value.toString().match(/^\d+(\.\d+)?$/)
    ) {
  ...
  return (
    <div className="relative w-full text-lg text-black">
      <div className="relative w-full">
        ...
        />
        <div
          className="absolute inset-y-0 right-0 flex px-0 py-0 mx-0 my-0"
          ref={ref}
        >
          {unit && (
            <div
              className={classNames(
                "self-center hidden px-4 whitespace-nowrap text-9B9B9B text-sm font-normal xs:block",
                unitClass
              )}
            >
              {unit}
      ...
      {error && <p className="mt-1 text-sm text-FA5C2F">{error}</p>}
    </div>
  );
};
