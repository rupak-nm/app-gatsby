...
import { LiquidityGaugePoolsList } from "@/modules/pools/liquidity-gauge-pools/LiquidityGaugePoolsList";
...
import { SORT_DATA_TYPES, SORT_TYPES, sorter } from "@/utils/sorting";
...
export const LiquidityGaugePoolsPage = () => {
  ...
  return (
    <Container
      className="pt-7 md:pt-16 pb-36"
      data-testid="liquidity-gauge-pools-page-container"
    >
      <div className="flex justify-end mb-7 md:mb-14">
        <div className="items-start justify-between w-full md:items-center sm:flex">
          <Link
            to={Routes.LiquidityGaugePoolsTransactions}
      ...
      <div className="flex justify-end font-semibold text-4E7DD9 text-md mb-[18px]">
        <Link to="/governance">View the Latest Gauge --&gt;</Link>
      </div>
...
function Content({
  ...
  if (data.length) {
    return <LiquidityGaugePoolsList pools={data} />;
  }
