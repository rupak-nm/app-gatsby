  ...
  return (
    <td className="max-w-sm px-6 py-6">
      <div className="flex items-center gap-2 w-max">
        <TableRowCoverAvatar imgs={row.imgSrc} />
    ...
    <td className="max-w-sm px-6 py-6 text-right">
      <div className="flex items-center justify-end w-full text-sm leading-6 whitespace-nowrap">
        ...
        <button
          className="p-1 ml-3"
          onClick={() => {
            return register(row.token, tokenSymbol, tokenDecimals);
          }}
        ...
        <Tooltip.Root>
          ...
          <Tooltip.Content side="top">
            <div className="max-w-sm p-3 text-sm leading-6 text-white bg-black rounded-xl">
              <p>{DateLib.toLongDateFormat(row.blockTimestamp, "UTC")}</p>
            </div>
  ...
  return [
    {
      ...
      align: "left",
      renderHeader: (col) => {
        return renderHeader(col, "blockTimestamp", sorts, handleSort);
      },
  ...
  const updateData = useMemo(() => {
    return data.map((txData) => {
      ...
      const projectName =
        ...
        coverData?.coverInfoDetails?.projectName;
      const pool = pools.find((x) => {
        return x.key === txData.key;
      });
