...
import { convertFromUnits, convertToUnits, toBN } from "@/utils/bn";
...
export const ReceiveAndUnlockModal = ({
  ...
  const [isReceiveRewardsModalOpen, setIsReceiveRewardsModalOpen] =
    useState(true);
  ...
  const handleChooseMax = () => {
    setInputValue(
      convertFromUnits(poolStaked, stakingTokenDecimals).toString()
    );
  };

  const { canWithdraw, handleWithdraw, withdrawing, error } =
    useLiquidityGaugePoolWithdraw({
      isExit,
      stakingTokenSymbol,
      stakingTokenDecimals,
      amountInUnits: convertToUnits(
        inputValue || "0",
        stakingTokenDecimals
      ).toString(),
      poolAddress,
  ...
  const btnClass =
    "w-full p-3 mt-6 font-semibold uppercase sm:min-w-auto sm:w-full";
        ...
        <div className="px-8 py-6 pb-8">
          <div className="mb-6">
            <button
              type="button"
              onClick={() => {
                return setIsReceiveRewardsModalOpen(true);
              }}
              className={classNames(
                "px-4 py-2 text-sm font-semibold text-primary rounded-big",
            ...
            <button
              type="button"
              onClick={() => {
                return setIsReceiveRewardsModalOpen(false);
              }}
              className={classNames(
                "px-4 py-2 text-sm font-semibold text-primary rounded-big",
                !isReceiveRewardsModalOpen && "bg-DEEAF6"
            ...
            {isReceiveRewardsModalOpen && (
              <div className="flex flex-row items-center justify-between text-sm">
                ...
                <span
                  ...
                  title={formattedStakedBalance.long}
                >
          ...
          {isReceiveRewardsModalOpen && (
            <RegularButton
              className={btnClass}
              disabled={
                !toBN(rewardAmount).isGreaterThan(0) || withdrawingRewards
              }
              onClick={() => {
...
export const ModalTitle = ({
  ...
  return (
    <Dialog.Title className="relative flex justify-center w-full px-3 py-4 font-semibold border-b border-b-B0C4DB">
