...
const renderDetails = (row) => {
  return <DetailsRenderer row={row} />;
};

const renderAmount = (row) => {
  return <PoolAmountRenderer row={row} />;
};
  ...
  return [
    {
      ...
      align: "left",
      renderHeader: (col) => {
        return renderHeader(col, "createdAtTimestamp", sorts, handleSort);
      },
...
const DetailsRenderer = ({ row }) => {
  const imgSrc = [
    {
      src: getTokenImgSrc(row.pool.stakingTokenSymbol),
      alt: "staking token logo",
    },
    {
      src: getTokenImgSrc(row.pool.rewardTokenSymbol),
  ...
  if (row.type === "RewardsWithdrawn") {
    const harvestAmountWithSymbol = (
      <TokenAmountSpan
        className="text-sm leading-5 text-01052D"
        amountInUnits={row.rewards}
        symbol={row.pool.rewardTokenSymbol}
        decimals={row.pool.rewardTokenDecimals}
      />
    );
  ...
  return (
    <td className="max-w-md px-6 py-6">
      <div className="flex items-center w-max">
        <TableRowCoverAvatar imgs={imgSrc} />
    ...
    <td className="max-w-sm px-6 py-6 text-right">
      <div className="flex items-center justify-end w-full text-sm leading-6 whitespace-nowrap">
        ...
        <button
          className="p-1 ml-3"
          onClick={() => {
            return register(row.token, data.symbol, data.tokenDecimals);
          }}
        ...
        <Tooltip.Root>
          ...
          <Tooltip.Content side="top">
            <div className="max-w-sm p-3 text-sm leading-6 text-white bg-black rounded-xl">
              <p>{DateLib.toLongDateFormat(row.createdAtTimestamp, "UTC")}</p>
            </div>
