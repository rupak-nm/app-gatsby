...
import { getUrlFromQueryObject } from "@/utils/url";
...
export const AvailableCovers = () => {
  ...
  const getSelectedViewOption = (query) => {
    const selectedView =
      typeof query[ViewQueryParam] === "string"
        ? query[ViewQueryParam]
        : defaultViewOption.value;

    return (
      viewOptions.find((item) => {
        return item.value === selectedView;
      }) || defaultViewOption
    );
  };
  ...
  const getSelectedSortOption = (query) => {
    const selectedSort =
      typeof query[SortQueryParam] === "string"
        ? query[SortQueryParam]
  ...
  const searchTerm =
    typeof query[SearchQueryParam] === "string" ? query[SearchQueryParam] : "";
      ...
      try {
        ...
        const text =
          (isDiversifiedProduct
            ...
            : item?.coverInfoDetails?.coverName ||
              item?.coverInfoDetails?.projectName) || "";
  ...
  const handleViewFilterChange = (option) => {
    ...
    const url = getUrlFromQueryObject(newUrl.query);
    replace(url);
  };
  ...
  return (
    <Container className="py-16" data-testid="available-covers-container">
      ...
      <Checkbox
        ...
        onChange={() => {
          return setShowDisabled(!showDisabled);
        }}
      >
...
function Content({
  ...
  return sortedCoversOrProducts.map((c) => {
    // if (idx > showCount - 1) return null

    if (
      selectedViewOption.value === SORT_TYPES.ALL &&
