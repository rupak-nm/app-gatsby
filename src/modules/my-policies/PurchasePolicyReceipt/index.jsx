...
export const PurchasePolicyReceipt = ({ txHash }) => {
  ...
  const { data: event, loading: eventLoading } = useFetchCoverPurchasedEvent({
    txHash,
  });
  ...
  const date = new Date(parseInt(event.blockTimestamp) * 1000).toUTCString();
  ...
  const startsAt = DateLib.getEodInUTC(
    DateLib.fromUnix(
      sumOf(event.blockTimestamp, coverOrProductData.coverageLag)
    )
  );
  ...
  const about = isDiversified
    ? coverOrProductData?.productInfoDetails?.about
    : coverOrProductData?.coverInfoDetails?.about;
  const parameters = isDiversified
    ? coverOrProductData?.productInfoDetails?.parameters
    : coverOrProductData?.coverInfoDetails?.parameters;
  const exclusions = isDiversified
    ? coverOrProductData?.productInfoDetails?.exclusions
    : coverOrProductData?.coverInfoDetails?.exclusions;
  const text = {
    policyInfo: about,
    coverRules: [
      ...
      <CoverParameters
        key="cover_params"
        textClassName="text-md sm:text-lg"
        parameters={parameters}
        titleClassName="text-md sm:text-lg font-bold mt-6 leading-5 mb-2 font-arial"
      />,
    ],
    exclusions,
  };
    ...
    <div className="bg-white font-arial">
      <div className="px-4 pt-4 m-auto sm:px-10 md:px-10 lg:max-w-5xl pb-52">
        <div className="flex flex-col text-center cursor-pointer sm:flex-row mt-9">
          <Link to={Routes.Home} replace className="sm:w-auto">
            <picture>
              <img
                loading="lazy"
        ...
        {policyReceiptData.map(({ label, value, valueClassName }, i) => {
          return (
            <div
              className="flex flex-col mt-2 leading-7 sm:flex-row sm:mt-4 text-md sm:text-lg"
              key={`${i}-${label}`}
            >
        ...
        <div className="mt-6 sm:mt-10">
          <div className="text-lg leading-6 mt-3.5 mb-4 sm:mb-10">
            <p className="mb-2 mr-2 font-bold leading-7 text-md sm:text-display-xs">
              On Behalf Of
            </p>
          ...
          {onBehalfOfData.map(({ label, value }, i) => {
            return (
              ...
              >
                <p className="font-bold leading-5 sm:flex-shrink-0 sm:w-1/2 md:w-full sm:max-w-60">
                  {label}
                </p>
          ...
          <div className="flex flex-col mt-6 leading-5 sm:flex-row sm:mt-10 text-md sm:text-lg">
            <p className="flex-shrink-0 font-bold sm:w-1/2 md:w-full max-w-60">
              Your {"cx" + liquidityTokenSymbol} Address
            </p>
        ...
        <Alert printable>
          <p className="text-sm font-bold sm:text-lg text-E03636">
            Beta Version Disclaimer
          </p>
          <p className="mt-1 text-sm sm:text-lg text-E03636 sm:leading-6">
            As you are participating in the beta version of the Neptune Mutual
            protocol, it is possible that the terms and exclusions may change.
          </p>
        </Alert>
