...
const CelerApiError = ({ message, className = "" }) => {
  ...
  return (
    ...
    >
      {splitted.length > 1 ? (
        <>
          <span className="mt-2 font-bold">{splitted[0]}: </span>{" "}
          <span>{splitted.slice(1).join(": ")}</span>
        </>
      ) : (
        message
      )}
...
export const CelerBridgeModule = ({
  ...
  const srcChainConfig = chains.find((x) => {
    return x.chainId === `0x${networkId.toString(16)}`;
  });

  useEffect(() => {
    const receiveAmount = convertFromUnits(
      estimation?.estimated_receive_amt || "0",
      destinationTokenDecimal
    ).toString();
    ...
    const baseFee = convertFromUnits(
    ...
    setInfoArray([
      ...
      },
      {
        ...
        info: (
          <>
            Estimated Gas fee for current chain is{" "}
            {formattedCurrentChainGas.long}.
      ...
      },
      {
        key:
          estimating ||
          toBNSafe(estimation.protocolFeePercent).isLessThanOrEqualTo(0)
            ? "Protocol Fee"
            : `Protocol Fee (${toBNSafe(estimation.protocolFeePercent)
                .decimalPlaces(4)
                .toString()}%)`,
  ...
  if (selectedBridge && selectedBridge !== BRIDGE_KEYS.CELER) {
    return <></>;
  }
      ...
      <div className="relative mt-4">
        ...
        <div className="relative mt-4">
          ...
          <NetworkSelect
            ...
            options={filteredNetworks.filter((n) => {
              return n.chainId.toString() !== networkId.toString();
            })}
            onChange={(val) => {
              return setSelectedNetworks((prev) => {
                return { ...prev, destNetwork: val };
              });
            }}
          />
        </div>
