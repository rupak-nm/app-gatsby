  ...
  return [
    {
      ...
      align: "left",
      renderHeader: (col) => {
        return renderHeader(col, "timestamp", sorts, handleSort);
      },
...
export const MyTransactionsTable = () => {
  ...
  const { sorts, handleSort, sortedData } = useSortData({
  ...
  return (
    <>
      <TableWrapper data-testid="policy-txs-table-wrapper">
        <Table>
          <THead columns={columns} data-testid="policy-txs-table-header" />
          {account ? (
            <TBody columns={columns} data={sortedData} />
          ) : (
...
const DetailsRenderer = ({ row }) => {
  ...
  return (
    <td
      className="w-auto px-6 py-6 text-sm leading-5 text-01052D"
      data-testid="details-col"
    >
...
const AmountRenderer = ({ row }) => {
  ...
  return (
    <td
      className="max-w-sm px-6 py-6 text-sm leading-6 text-right min-w-120 text-01052D"
