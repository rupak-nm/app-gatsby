  ...
  return (
    <Container className="py-16" data-testid="page-container">
      <div className="flex justify-end">
        <Link
          to={Routes.LiquidityTransactions}
  ...
  if (loading || isSummaryLoading) {
    return (
      <Grid className="mb-24 mt-14" data-testid="loading-grid">
        <CardSkeleton numberOfCards={15} statusBadge={false} subTitle={false} />
      </Grid>
    );
  }
  ...
  return (
    <Grid className="mb-24 mt-14" data-testid="liquidities-grid">
      {data.map((x) => {
        ...
        return (
          <Link
            to={Routes.MyCoverLiquidity(coverKey)}
            key={x.id}
          ...
          >
            <MyLiquidityCoverCard
              ...
              subProducts={
                isDiversified ? getProductsByCoverKey(coverKey) : null
              }
            />
          </Link>
        );
