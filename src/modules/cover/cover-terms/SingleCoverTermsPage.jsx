...
export const SingleCoverTermsPage = ({ loading, coverOrProductData }) => {
  ...
  const name = isDiversifiedProduct
  ...
  const blockchains = isDiversifiedProduct
    ? coverOrProductData?.productInfoDetails?.blockchains
    : coverOrProductData?.coverInfoDetails.blockchains;
  const about = isDiversifiedProduct
    ? coverOrProductData?.productInfoDetails?.about
    : coverOrProductData?.coverInfoDetails.about;
  const parameters = isDiversifiedProduct
    ? coverOrProductData?.productInfoDetails?.parameters
    : coverOrProductData?.coverInfoDetails.parameters;

  return (
    <div>
      <Link to={Routes.Home} replace className="block w-max">
        <picture>
          <img
            loading="lazy"
