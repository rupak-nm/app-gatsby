...
const UnlockEscrow = ({
  ...
  const penaltyAmount = isPrematureUnlock
    ? toBN(veNPMBalance)
        .multipliedBy(PREMATURE_UNLOCK_PENALTY_FRACTION)
        .toString()
    : "0";

  const receiveAmount = toBN(veNPMBalance).minus(penaltyAmount).toString();

  const formattedPenaltyAmount = formatCurrency(
    convertFromUnits(penaltyAmount, NPMTokenDecimals),
    router.locale,
    NPMTokenSymbol,
    true
  );
  const formattedReceiveAmount = formatCurrency(
    convertFromUnits(receiveAmount, NPMTokenDecimals),
    router.locale,
    NPMTokenSymbol,
    true
  );
  ...
  return (
    <VoteEscrowCard>
      ...
      <div className="p-8">
        ...
        <button
          className="uppercase flex items-center gap-2.5 font-semibold text-xs leading-6"
          onClick={() => {
            onBack();
          }}
        >
