...
import { convertFromUnits, convertToUnits, toBN, toBNSafe } from "@/utils/bn";
import {
  calculateBoost,
  getBoostText,
  getBoostTextClass,
...
const VoteEscrow = () => {
  ...
  const { NPMTokenDecimals, NPMTokenSymbol, NPMTokenAddress } =
    useAppConstants();
  ...
  const oldLockDurationBN = toBNSafe(data.unlockTimestamp).isGreaterThan(
    DateLib.unix()
  )
  ...
  const boost = toBN(calculateBoost(lockDuration))
  ...
  const formattedLockedNpm = formatCurrency(
    convertFromUnits(newLockedNpm, NPMTokenDecimals),
    router.locale,
    NPMTokenSymbol,
    true
  );
  ...
  const formattedVotingPower = formatCurrency(
    convertFromUnits(votingPower, NPMTokenDecimals),
    router.locale,
    NPMTokenSymbol,
    true
  );
  ...
  const sliderDisplayValue =
    sliderValue || oldDurationInWeeks || VOTE_ESCROW_MIN_WEEKS;
  const unlockDate = sliderValue
    ? DateLib.addDays(new Date(), sliderValue * 7)
    : DateLib.fromUnix(data.unlockTimestamp);
  ...
  const buttonDisabled =
    locking || !!loadingMessage || !(agreed && ((!extend && input) || extend));
      ...
      <div className="flex items-center justify-between mt-6 mb-4">
        ...
        <div className="flex items-center text-sm">
          <Checkbox
            ...
            className="w-4 h-4 m-0 border-gray-300 border-1 rounded-1"
            id="extend-checkbox"
            labelClassName="ml-1"
          >
            Extend Only
  ...
  return (
    <div className="max-w-[990px] mx-auto">
      <VoteEscrowCard className="!max-w-full p-5 md:p-8 rounded-2xl flex flex-wrap gap-4 justify-between items-end mb-6">
        <div>
          <h2 className="mb-1 font-semibold text-display-sm text-4E7DD9">
            Get Vote Escrow NPM
          </h2>
          <p className="text-sm">
            Get boosted voting power and boosted gauge emissions
          </p>
        </div>
        <div className="flex flex-wrap gap-4">
          <Link
            to={Routes.LiquidityGaugePools}
      ...
      <VoteEscrowCard className="!max-w-full grid grid-cols-1 lg:grid-cols-2 gap-8 p-5 md:p-8">
        ...
        <div>
          <div className="p-4 rounded-lg md:p-6 bg-F3F5F7">
            <div className="mb-4">
              <GaugeChartSemiCircle
                chartDiameter={isMobile ? 240 : undefined}
                min={1}
                max={4}
                value={boost}
              />
            ...
            <Slider
              ...
              onChange={(value) => {
                if (value >= oldDurationInWeeks) {
                  // only allows increasing the duration
            ...
            <div className="flex justify-between mb-8 text-sm">
              ...
              <div>
                <div className="text-xs text-right">Unlocks on:</div>
            ...
            <div className="grid grid-cols-[auto_1fr] gap-2 mb-4">
              <Checkbox
                ...
                className="w-4 h-4 m-0 border-gray-300 border-1 rounded-1"
                id="agree-terms-escrow"
              />
              <label htmlFor="agree-terms-escrow" className="-mt-0.5 text-sm">
                I hereby acknowledge my obligation to pay a penalty fee of{" "}
                {formatPercent(PREMATURE_UNLOCK_PENALTY_FRACTION)} in the event
                that I prematurely unlock, as per the applicable{" "}
                <a
                  href="https://neptunemutual.com/policies/standard-terms-and-conditions/"
                  target="_blank"
                  className="text-1170FF"
                  rel="noreferrer"
                >
          ...
          <div className="mt-6 text-right">
            <button
              disabled={!canUnlock}
              className={classNames(
                "text-4E7DD9 text-sm font-semibold",
                !canUnlock ? "opacity-50 cursor-not-allowed" : ""
              )}
              onClick={() => {
                document
                  .querySelector("#vote-escrow-page")
