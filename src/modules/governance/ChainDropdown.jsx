...
const ChainDropdown = ({
  ...
  const allChainSelected = options.every((chainId) => {
    return selected.includes(chainId.value);
  });
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div
          ref={ref}
          className="absolute z-10 top-[calc(100%+8px)] bg-white left-0 shadow-xl border-1 border-D0D5DD w-[238px] rounded-2"
        >
          <div className="m-2.5 flex items-center">
            <input
              className="w-full px-4 py-2 text-sm leading-5 outline-none"
              value={search}
              onChange={(e) => {
              ...
              .map((option) => {
                return (
                  <button
                    ...
                    className="py-2.5 px-4 w-full flex items-center gap-1 text-sm hover:bg-EEEEEE cursor-pointer"
                    key={option.value}
                  >
                    <Checkbox
          ...
          <div className="p-4">
            <OutlinedButton
              className="w-full text-sm font-semibold normal-case border-D0D5DD text-344054 hover:text-344054 rounded-2"
              onClick={() => {
                onSelectionChange([]);
              }}
            >
