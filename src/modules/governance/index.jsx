...
export const GovernanceSinglePage = () => {
  ...
  const {
    ...
    const filteredResults = getResultsByChains(
      getVotingResults(proposalDetail.choices, proposalDetail.scores),
      selectedChains
    );

    const percentSum = sumOf(
      ...filteredResults.map((x) => {
        return x.percent;
      })
    );
    const emissionOfSelectedChains = toBN(EMISSION_PER_EPOCH)
      .multipliedBy(percentSum)
      .toString();

    const distribution = filteredResults.map((result) => {
      return {
        key: result.key,
        emission: toBN(EMISSION_PER_EPOCH)
          .multipliedBy(result.percent)
          .toString(),
      };
    });
