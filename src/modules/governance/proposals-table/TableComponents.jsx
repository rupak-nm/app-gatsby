...
import { getProposalLink, snapshotColors } from "@/utils/snapshot";
import { Trans } from "@lingui/macro";

export const WhenRenderer = ({ row, locale }) => {
  return (
    <td className="px-6 py-4 text-sm w-42 whitespace-nowrap">
      <InfoTooltip
        infoComponent={DateLib.toLongDateFormat(row.start / 1000, locale)}
      >
        <span>{fromNow(row.start)}</span>
      </InfoTooltip>
    </td>
  );
};
  ...
  return (
    <span
      className={classNames(
        "py-0.5 px-1.5 text-xs rounded-2xl",
...
export const TypeRenderer = ({ row }) => {
  return (
    <td className="w-24 px-6 py-4 text-xs whitespace-nowrap">
...
export const DetailsRenderer = ({ row }) => {
  return (
    <td className="px-6 py-4 text-sm min-w-345" data-testid="timestamp-col">
      {row.title}
    </td>
  );
};
...
export const TagRenderer = ({ row }) => {
  return (
    <td className="px-6 py-4 text-xs w-28 whitespace-nowrap">
      {row.category ? (
...
export const ResultRenderer = ({ row }) => {
  return (
    <td className="px-6 py-4 w-330px whitespace-nowrap">
      <Results results={row.scores} />
    </td>
  );
};
  ...
  return (
    <div className="border border-t-0 border-B0C4DB rounded-b-xl bg-white px-6 pt-3 pb-4 flex flex-col sm:flex-row items-start sm:items-center justify-end gap-2.5">
      <div className="flex items-center gap-4 text-sm">
        <label className="opacity-40" htmlFor="rows-per-page">
          Rows per page
        </label>
