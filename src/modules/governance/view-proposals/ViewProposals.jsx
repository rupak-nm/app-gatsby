...
export const ViewProposals = () => {
  ...
  const lockDuration = toBNSafe(data.unlockTimestamp).isGreaterThan(
    DateLib.unix()
  )
    ? toBNSafe(data.unlockTimestamp)
        .minus(DateLib.unix()) // to duration left
        .decimalPlaces(0, BigNumber.ROUND_CEIL) // rounding
        .toNumber()
    : 0;

  const boost = toBNSafe(calculateBoost(lockDuration))
  ...
  const formattedVeNPMBalance = formatCurrency(
    convertFromUnits(data.veNPMBalance, NPMTokenDecimals),
    router.locale,
    veNPMTokenSymbol,
    true
  );
    ...
    <div className="flex flex-col items-center gap-8 p-8 bg-white border lg:flex-row rounded-2xl border-B0C4DB">
      <div className="flex-1 w-full">
        ...
        <div className="p-6 mt-6 break-all rounded-2 bg-F3F5F7 md:break-words">
          <KeyVal heading="Account" value={account || "N/A"} />
