...
const LiquidityGauge = ({
  ...
  const { allZeros, data } = useMemo(() => {
    ...
    const allZeros = !results.find((s) => {
      return toBNSafe(s.percent).isGreaterThan(0);
    });
  ...
  const formattedEmission = formatCurrency(
    convertFromUnits(emission, NPMTokenDecimals),
    router.locale,
    NPMTokenSymbol,
    true
  );
    ...
    <GovernanceCard className="gap-6 p-4 md:p-8">
      ...
      <div className="relative -my-5 gauge-chart-liquidity md:my-0">
        ...
        <div className="absolute top-[50%] left-[50%] max-w-[150px] md:max-w-[unset] translate-x-[-50%] translate-y-[-50%] text-center">
          <div className="font-bold text-md md:text-display-sm">
            <Trans>Liquidity Gauge</Trans>
          </div>
          <div className="text-sm font-medium md:text-md">
            <Trans>Epoch Emission:</Trans> {formattedEmission.long}
          </div>
        </div>
      </div>
      ...
      <div className="mt-8 text-center">
        ...
        <div
          ...
          title={DateLib.toLongDateFormat(asOfDate, router.locale)}
        >
          As of: {formattedDate}
        </div>
      </div>
          ...
          return (
            <div
              ...
              style={{
                ...
                opacity:
                  mouseEnteredOnLegend && hoveredName !== item.name
                    ? "0.2"
                    : undefined,
                transition: "all 0.3s",
              }}
