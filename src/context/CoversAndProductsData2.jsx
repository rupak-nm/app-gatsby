...
import { convertToUnits, toBNSafe } from "@/utils/bn";
import { useWeb3React } from "@web3-react/core";

const CoversAndProductsDataContext = React.createContext({
  loading: false,
  data: [],

  getCoverOrProduct: (_coverKey, _productKey) => {
    return null;
  },

  getCoverByCoverKey: (_coverKey) => {
    return null;
  },

  getProductsByCoverKey: (_coverKey) => {
    return null;
  },

  getProduct: (_coverKey, _productKey) => {
    return null;
  },
...
export const CoversAndProductsProvider2 = ({ children }) => {
  ...
  const stablecoinDecimals =
    ChainConfig[networkId]?.stablecoin.tokenDecimals ?? 6;
  ...
  const updateData = useCallback(
    async function () {
      try {
        const _data = account
          ? await getProductSummaryWithAccount(networkId, account)
          : await getProductSummary(networkId);
        ...
        setData(
          _data
            ...
            .map((x) => {
              return {
                ...x,
                availableForUnderwriting: convertToUnits(
                  x.availableForUnderwriting,
                  stablecoinDecimals
                ).toString(),
                capacity: convertToUnits(
                  x.capacity,
                  stablecoinDecimals
                ).toString(),
                commitment: convertToUnits(
                  x.commitment,
                  stablecoinDecimals
                ).toString(),
                minReportingStake: convertToUnits(
                  x.minReportingStake,
                  npmDecimals
                ).toString(),
                reassurance: convertToUnits(
                  x.reassurance,
                  stablecoinDecimals
                ).toString(),
            ...
            .sort((a, b) => {
              const text1 =
                a?.productInfoDetails?.productName ||
                a?.coverInfoDetails?.coverName ||
                a?.coverInfoDetails?.projectName ||
                "";
              const text2 =
                b?.productInfoDetails?.productName ||
                b?.coverInfoDetails?.coverName ||
                b?.coverInfoDetails?.projectName ||
                "";
  ...
  const getCoverOrProduct = (coverKey, productKey) => {
    return data.find((x) => {
      const isSameProductKey = isValidProduct(productKey)
  ...
  const getDedicatedCovers = () => {
    return data.filter((x) => {
      return !x.productKey && !x.supportsProducts;
    });
  };
  ...
  const getCoverByCoverKey = (coverKey) => {
    return data.find((x) => {
      return x.coverKey === coverKey && !x.productKey;
    });
  };
  ...
  const getProduct = (coverKey, productKey) => {
    return data.find((x) => {
      return x.coverKey === coverKey && x.productKey === productKey;
    });
  };
