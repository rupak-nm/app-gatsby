      ...
      <Tooltip.Root delayDuration={200}>
        ...
        >
          ...
          <span className="text-xs font-light leading-4">
            Your transaction statuses will be collected in this tray. Feel free
            to navigate through the screens while you wait.
          </span>
        </Tooltip.Content>
      </Tooltip.Root>
    );
  ...
  return (
    <>
      ...
      <header className="sticky z-40 bg-black -top-px text-EEEEEE print:hidden">
        <NavContainer>
          <nav
            className="flex justify-between max-w-full mx-auto"
            aria-label="Top"
          >
                  ...
                  return (
                    <Link
                      ...
                      className={classNames(
                        ...
                        !link.active &&
                          "before:w-full before:h-1 before:-bottom-1 before:left-0 before:absolute before:bg-primary before:scale-x-0 before:focus-visible:scale-x-100 before:transition-all before:hover:scale-x-100"
                      )}
                    >
                      {link.name}
                    </Link>
              ...
              <div className="relative flex ml-3">
                <TransactionOverviewTooltip hide={isTxDetailsPopupOpen}>
                  <button
                    ...
                    onClick={() => {
                      return setIsTxDetailsPopupOpen((val) => {
                        return !val;
                      });
                    }}
                  ...
                  >
                    <span className="sr-only">
                      <Trans>transaction overview button</Trans>
                    </span>
...
export const MenuModal = ({
  ...
  useEffect(() => {
    ...
    router.events.on("PUSH", handleRouteNavigate);
      ...
      {({ open }) => {
        return (
          <>
            ...
            <Menu.Items className="absolute z-10 w-auto py-4 space-y-6 overflow-y-auto text-white bg-black shadow-lg outline-none -left-1/3 min-w-205">
              {items.map((item, idx) => {
                return (
                  <Menu.Item key={idx}>
                    {({ active }) => {
