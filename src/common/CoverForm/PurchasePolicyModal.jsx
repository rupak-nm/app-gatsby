 ...
 * @returns
 */
export const PurchasePolicyModal = ({ isOpen, txHash, amount }) => {
  ...
  return (
    ...
    >
      <ModalWrapper
        className={classNames("max-w-lg bg-white", txHash && "!p-0")}
      >
        {txHash ? (
          <Complete
            txHash={txHash}
            onClose={hanldeClose}
            amountInDollars={finalAmount.current}
          />
        ) : (
          <Loading />
        )}
...
function Complete({ txHash, onClose, amountInDollars }) {
  ...
  const nftLink = isTestNet
    ? "https://test.nft.neptunemutual.com"
  ...
  return (
    <div className="flex flex-col items-center">
      <div className="flex gap-2.5 justify-center items-center p-8 bg-01052D text-white w-full">
        ...
        <h4 className="font-semibold text-display-xs">
          <Trans>Cover Purchased Successfully!</Trans>
        </h4>
      </div>

      <div className="p-8 pt-3 pb-7">
        <img
          src="/images/nft-image.webp"
          alt="image showing different NFT characters"
        />
        ...
        <p className="mt-6 text-center text-md">
          {isBinanceSmartChain
            ? "You've unlocked an NFT. Head to our NFT portal to mint it."
            : "You are on fire! Head to our NFT Portal to view your progress."}
        </p>

        <div className="flex items-center justify-center gap-6 mt-8 center">
          <Link
            to={Routes.ViewPolicyReceipt(txHash)}
            target="_blank"
            // onClick={onClose}
