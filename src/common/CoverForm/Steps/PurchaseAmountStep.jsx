...
const PurchaseAmountStep = ({
  ...
  function handleChange(val) {
    ...
    if (isGreaterOrEqual(val, availableLiquidity)) {
      ...
      setError(
        t(i18n)`Maximum protection available is ${maxProtection}` +
          ". Choose a amount less than available."
      );
    } else if (isGreater(val, MAX_PROPOSAL_AMOUNT)) {
      const maxThreshold = formatCurrency(
        MAX_PROPOSAL_AMOUNT,
        router.locale,
        liquidityTokenSymbol,
        true
      ...
      const minThreshold = formatCurrency(
        MIN_PROPOSAL_AMOUNT,
        router.locale,
        liquidityTokenSymbol,
        true
  ...
  return (
    <>
      ...
      <p className="mt-1 mb-8 text-lg text-center text-999BAB">
        <Trans>
          Don&apos;t worry, you&apos;re not required to make a purchase just
          yet.
        </Trans>
      </p>
      ...
      />
      {error && error !== "Please connect your wallet" && (
