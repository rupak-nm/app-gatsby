...
const PurchasePolicyStep = ({
  ...
  return (
    <div>
      <p className="text-lg text-center text-999BAB">
        <Trans>You Will Pay</Trans>
      </p>
      <div
        className="flex justify-center mt-1 mb-8 font-bold text-center text-display-md text-4E7DD9"
        title={
          formatCurrency(coverFee, router.locale, liquidityTokenSymbol, true)
            .long
        }
      >
        {updatingFee ? (
        ...
        <p className="flex items-center">
          {
            formatCurrency(
              value,
              router.locale,
              "cx" + liquidityTokenSymbol,
              true
            ).long
          }{" "}
          (Claimable {liquidityTokenSymbol} Token)
          <CxUsdToolTip
      ...
      {error && error === "Insufficient Balance" && (
        <Alert className="flex items-center text-FA5C2F">
          <Trans>
            Your balance is not enough to pay the fee. Please reload your
            wallet.
          </Trans>
        </Alert>
      )}
      ...
      <div className="w-full px-2 py-6 mt-8 rounded-lg md:px-8 bg-F3F5F7">
          ...
          {!editForm && (
            <OutlinedButton
              ...
              className={classNames(
                "rounded-md !py-1 mt-3 md:mt-0",
                approving && "opacity-60 cursor-not-allowed"
              )}
              onClick={!approving && handleEditForm}
            >
              Edit
            </OutlinedButton>
          )}
        ...
        <p className="mt-8 mb-4 text-lg font-semibold uppercase">
        ...
        <div className="mt-6">
          ...
          {editForm && (
            <div className="relative flex mt-11">
              <div
                className="absolute h-2 bg-999BAB bg-opacity-30 top-1.5"
                style={{ width: "calc(100% - 20px)" }}
              />
              <div
                className="absolute h-2 bg-4E7DD9 top-1.5"
                style={{ width: `calc(0% + ${radioProgress}%)` }}
              />
              <CustomRadio
    ...
    <Tooltip.Root>
      ...
      <Tooltip.Content side="right">
        <div className="w-full p-2 text-xs tracking-normal bg-black rounded-lg max-w-70 text-EEEEEE">
          <p>
            <Trans>
              You will receive cx{liquidityTokenSymbol} or Claimable{" "}
              {liquidityTokenSymbol} upon successful completion of this
              transaction.
            </Trans>
          </p>
          <p className="mt-6">
            <Trans>
              The cx{liquidityTokenSymbol} token will be redeemable for{" "}
              {liquidityTokenSymbol} at a 1:1 ratio if {projectOrProductName}{" "}
              cover resolves as “Incident Occured”
            </Trans>
          </p>
        </div>
