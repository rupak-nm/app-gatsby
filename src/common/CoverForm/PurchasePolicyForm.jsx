...
const getCoveragePeriodLabels = (locale) => {
  ...
  if (day >= 25) {
    return [
      monthNames[(currentMonthIndex + 1) % 12] +
        getMonthEnd(currentMonthIndex + 1, fullYear),
      monthNames[(currentMonthIndex + 2) % 12] +
        getMonthEnd(currentMonthIndex + 2, fullYear),
      monthNames[(currentMonthIndex + 3) % 12] +
        getMonthEnd(currentMonthIndex + 3, fullYear),
    ];
  }
  ...
  return [
    monthNames[(currentMonthIndex + 0) % 12] +
      " " +
      getMonthEnd(currentMonthIndex + 0, fullYear),
    monthNames[(currentMonthIndex + 1) % 12] +
      " " +
      getMonthEnd(currentMonthIndex + 1, fullYear),
    monthNames[(currentMonthIndex + 2) % 12] +
...
export const PurchasePolicyForm = ({
  ...
  const [isReferralCodeCheckPending, setIsReferralCodeCheckPending] =
    useState(false);
  ...
  return (
    <div
      className="flex flex-col w-616"
      data-testid="purchase-policy-form-container"
    >
      {formSteps === 0 && value && <StepsIndicator completed="50" />}
      {formSteps === 1 && (
        <StepsIndicator completed={value && coverMonth ? "100" : "50"} />
      )}
      <div
        className="w-full p-4 rounded-xl bg-FEFEFF md:p-9 border-B0C4DB border-1.5"
        data-testid="purchase-policy-form"
      >
        ...
        {formSteps === 2 && (
          <>
            ...
            >
              <Trans>
                I have read, understood, and agree to the cover terms,
                parameters, and exclusions, as well as the standard exclusions.
              </Trans>
            </Checkbox>
          </>
        )}
            ...
            <div className="flex flex-wrap items-end justify-between ml-4">
              ...
              <ConnectWallet networkId={networkId} notifier={notifier}>
                {({ onOpen }) => {
                  return (
                    <RegularButton
                      className="py-1.5 px-2.5 text-sm h-fit"
                      onClick={onOpen}
                    >
                      Connect Wallet
                    </RegularButton>
        ...
        {policyStatus.disabled && (
          <Alert>
            <p>
              <Trans>Policy purchases on this product are disabled.</Trans>
            </p>
            <p>
              <Trans>Reason: {policyStatus.reason}</Trans>
            </p>
          </Alert>
        )}
          ...
          <div className="flex flex-wrap justify-end mt-12 xs:flex-row-reverse sm:justify-start">
            ...
            {formSteps === 0 && (
              <OutlinedButton
                ...
                className={classNames(
                  "text-[#01052D] hover:text-[#01052D] flex items-center py-3 px-4 rounded-big w-full sm:w-auto justify-center ml-4 mt-2 md:mt-0 !bg-E6EAEF border-none focus-visible:ring-E6EAEF "
                )}
              >
                <Trans>Cancel</Trans>
              </OutlinedButton>
            ...
            {formSteps > 0 && (
              <BackButton
                className={classNames(
                  "flex items-center py-3 px-4 rounded-big w-full sm:w-auto justify-center uppercase ml-4 mt-2 md:mt-0"
                )}
                onClick={() => {
                  return setFormSteps((prev) => {
                    return prev - 1;
                  });
                }}
              />
        ...
        <PurchasePolicyModal
          isOpen={purchaseWaiting || Boolean(txHash)}
          txHash={txHash}
