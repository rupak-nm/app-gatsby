...
export const ProductsGrid = () => {
  ...
  const { loading, getProductsByCoverKey, getCoverByCoverKey, getProduct } =
  ...
  const { searchValue, setSearchValue, filtered } = useSearchResults({
    ...
    filter: (data, searchTerm) => {
      ...
      const text = isDiversifiedProduct
        ...
        : data?.coverInfoDetails.coverName ||
          data?.coverInfoDetails.projectName;
  ...
  const linkToPath = useMemo(() => {
    const queryString = getUrlFromQueryObject(
      {
  ...
  return (
    <Container
      className="py-16"
      data-testid="available-covers-container"
      id="diversified-products-container"
    >
      <div className="flex flex-wrap items-center justify-between gap-6 md:flex-nowrap">
        <div className="flex items-center">
          ...
          <h1 className="font-bold text-display-xs lg:text-display-sm">
            {coverData?.coverInfoDetails?.coverName ||
              coverData?.coverInfoDetails?.productName ||
              ""}
          </h1>
        </div>
...
function Content({ data = [], loading = false, getProduct }) {
  if (loading) {
