...
import * as Router from "@/src/hooks/useRouter";
import { actions as coverActions } from "@/src/config/cover/actions";

const mockFunction = (file, method, returnData) => {
  jest.spyOn(file, method).mockImplementation(() => {
    return returnData;
  });
};
...
describe("CoverCard component", () => {
  ...
  test("should render correct number of cover-action cards", () => {
    ...
    const cardsLength = Object.keys(coverActions).filter((x) => {
      return x !== props.activeKey;
    }).length;
  ...
  test("should have correct cover-action card link", () => {
    ...
    const actions = Object.keys(coverActions).filter((x) => {
      return x !== props.activeKey;
    });
